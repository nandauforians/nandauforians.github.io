...
async function initializePage(bowler1Name, bowler2Name) {
    try {
        bowler1 = bowler1Name;
        bowler2 = bowler2Name;

        const data1 = await loadData(bowler1);
        const data2 = await loadData(bowler2);
        searchData = data1.concat(data2);

        // searchData.sort((a, b) => a.match.localeCompare(b.match));
...
function populateMatchIds(bowler1Data, bowler2Data) {
    ...
    const bowler2MatchIds = bowler2Data.map(row => row['match']);

    // Find common 'match' IDs between both datasets
    const commonMatchIds = bowler1MatchIds.filter(matchId => bowler2MatchIds.includes(matchId));

    // Populating the dropdown with unique Test Match IDs
    const uniqueCommonMatchIds = [...new Set(commonMatchIds)]; // Remove duplicates
    uniqueCommonMatchIds.forEach(testMatchId => {
        ...
        const bowler2MatchDetails = bowler2Data.find(row => row['match'] === testMatchId);

        // Construct description for the match
        const description = `Match: ${bowler1MatchDetails['match']}, ${bowler1MatchDetails['opponent']} @ ${bowler1MatchDetails['venue']}, Date: ${bowler1MatchDetails['matchDate']}`;
